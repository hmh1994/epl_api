...
def match_detail2(fixture_id: str, db: Session = Depends(get_db)):
    ...
    for key in ["home_lineup", "away_lineup", "home_substitutes", "away_substitutes", "home_substitutions", "away_substitutions"]:
        val = data.get(key)
        if val and isinstance(val, str):
            try:
                data[key] = json.loads(val)
...
@router.get("/infotest2/{fixture_id}")
def match_detail3(fixture_id: str, db: Session = Depends(get_db)):
    query = text("""
    WITH base AS (
        SELECT fx.id AS fixture_id, fx.home_team_score, fx.away_team_score, ma.id AS match_id,
               fx.home_team_id, fx.away_team_id
