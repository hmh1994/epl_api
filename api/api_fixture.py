...
WITH base AS (
    SELECT fx.id AS fixture_id, fx.home_team_score, fx.away_team_score, ma.id AS match_id,
           fx.home_team_id, fx.away_team_id, fx.kickoff_time
...
LEFT JOIN LATERAL (
    ...
    for key in ["home_lineup", "away_lineup", "home_substitutes", "away_substitutes", "home_team_recent_form", "away_team_recent_form"]:
        val = data.get(key)
    ...
    # game_stat 이 JSON 문자열로 온 경우 파싱
    game_stat = data.get("game_stat")
    if game_stat and isinstance(game_stat, str):
        try:
            data["game_stat"] = json.loads(game_stat)
        except Exception:
            pass
